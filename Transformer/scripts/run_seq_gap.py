...
parser.add_argument('--extra-cmd', type=str, default="", help='extra cmd, such as --short-train or --quick-save --no-epoch-checkpoints --sp-global-weight-sparsity or --sp-prune-threshold 0.3')
parser.add_argument('--extra-cmd-step0', type=str, default="", help='extra cmd, for step 0')
parser.add_argument('--sparsity-type', type=str, default="irregular", help='sparsity type, 4:2-H-V-balanced, irregular, block, etc')
parser.add_argument('--global-workspace', type=str, default=None, help='global workspace')
...
parser.add_argument('--resume-from-step', type=int, default=0, help='resume from step')
...
parser.add_argument('--partition-type', type=str, default=None, help='cyclic or random partition')
parser.add_argument('--precision', type=str, default="amp", help='fp16 or 32')
...
os.system("cp -r /home/prune_utils /workspace/translation/")
...
if args.global_workspace is None:
    global_workspace = f"/results/GaP/rand_{num_parts}_partition_seq_gap/{folder_name}/{sparsity}/multi_round_3_partition_sequential_gap_ep_{epoch_per_train}_per_train/"
else:
    global_workspace = args.global_workspace

os.system("mkdir -p {}".format(global_workspace))
local_workspace = [global_workspace + "/step_{}".format(args.resume_from_step-1)] #[global_workspace +  "/round_1/step_1/"]
...
for step in range(args.resume_from_step, args.resume_from_step + num_steps):

    os.system("nvidia-smi")

    if lr_sch_across_step == 'constant':
        lr = lr_const_policy(base_lr, -100, step, num_steps)
    ...
    if partition_type == "cyclic":
        s = step % num_parts
        config_file = f"{config_folder}/step_{s}.yaml"
    elif partition_type == "random":
        config_file = f"{config_folder}/step_{step}.yaml"
    else:
        print("Need a partition type in [cyclic, random]")
        sleep(10)
        exit()
    ...
    os.system(f"touch {local_workspace[-1]}/log.txt")
    extra_extra_cmd = f"2>&1 | tee {local_workspace[-1]}/log.txt"

    cmd.append(f"bash scripts/run_DGX1_AMP_8GPU.sh {prec} {seed} {lr} 4000 {epoch_per_train} 5120 8 {local_workspace[-1]} transformer_wmt_en_de_big_t2t \"--restart-training --resume {resume} --sp-retrain --sp-prune-before-retrain  --sp-admm-sparsity-type {sparsity_type} --sp-config-file {config_file} {extra_cmd}\" {extra_extra_cmd}")
    print(cmd[-1])
    os.system(cmd[-1])
