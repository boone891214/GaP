...
for step in range(num_steps):
    ...
    config_file = f"/home/profiles/rand_{num_parts}_parts_seq_gap_sp_{sparsity}/step_{step}.yaml"
    ...
    cmd.append(f"bash scripts/run_DGX1_AMP_8GPU.sh 'amp' {seed} {lr} 4000 {epoch_per_train} 5120 8 {local_workspace[-1]} transformer_wmt_en_de_big_t2t \"--restart-training --resume {resume} --sp-retrain --sp-prune-before-retrain  --sp-admm-sparsity-type {sparsity_type} --sp-config-file {config_file} {extra_cmd}\" {extra_extra_cmd}")
    print(cmd[-1])
    os.system(cmd[-1])
