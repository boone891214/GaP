...
def reshape_2d(matrix, r, c):
    from skimage.util.shape import view_as_windows
    np_matrix = matrix.detach().cpu().numpy()
    np_mat_as_windows = view_as_windows(np_matrix, (r,c), step=(r,c))
    np_mat_as_windows_4x4_flatten = np_mat_as_windows.reshape(np_mat_as_windows.shape[0], np_mat_as_windows.shape[1], -1)
    #np_mat_as_windows_hw_flatten = np_mat_as_windows_4x4_flatten.reshape(-1,np_mat_as_windows_4x4_flatten.shape[-1])
    #print(np_mat_as_windows_hw_flatten.shape)
    return torch.from_numpy(np_mat_as_windows_4x4_flatten).cuda()
    #pass

def reshape_2d_inv(matrix, m):
    pass
...
def mn_2d_best(matrix, m, n):
    ...
    mat = mat.view(matrix.shape[0]//m,matrix.shape[1]//m,m,m)
    mat = mat.permute(0,2,1,3).reshape(-1,mat.shape[1]*mat.shape[3])
