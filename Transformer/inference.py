...
from prune_utils.prune_main import prune_print_sparsity
import numpy as np
import sys
import os
import time
...
def main(args):
    ...
    for model in models:
        ...
        for name, W in model.named_parameters():
            print(name, W.shape)
            if "self_attn.in_proj_weight_q" in name:
                print(name, W.shape)
                print(W.detach().cpu().numpy().astype(np.float32)[0:4,0:4])
            #print(W[:10,:10])
        #input("Continue?")
        #exit()
    #input("Continue inference?")
    #print("What happend?")
    #exit()
    ...
    print('Translate time: {} s'.format(log_dict['total_infernece_time']), file=sys.stderr)
