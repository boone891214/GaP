...
def main(args):
    ...
    for name, W in model.named_parameters():
        ...
        if 'weight' in name:
            if len(W.detach().cpu().numpy().shape) > 1:
                with open('transformer_layer_names.yaml','a') as ff:
                    print(name, W.shape)
                    ff.write("{}: 0.8\n".format(name))
    exit()
    '''
    ...
    if args.combine_without_finetune:
        from prune_utils.admm import ADMM
        args.sp_admm_update_epoch = 1
        prune_algo = ADMM(args, trainer.model, logger=None)
        prune_algo.prune_harden()

        if os.path.isdir(args.save_dir) is False:
            os.system('mkdir -p ' + args.save_dir)
            print("New folder {} created...".format(args.save_dir))
        save_checkpoint(args, trainer, epoch_itr, val_loss=None)
        exit()
    ...
    for name, W in model.named_parameters():
        ...
        if 'encoder.layers.0.self_attn.in_proj_weight_q' in name:
            np_W = W.detach().cpu().numpy()
            print((np_W[128:128+32,128:128+32]!=0)+0.0)
    ...
    #exit()
    print(lr, args.min_lr)
    print(epoch_itr.epoch, max_epoch )
    print(trainer.get_num_updates() , max_update)
    print (current_bleu , tgt_bleu)
    while lr >= args.min_lr and epoch_itr.epoch < max_epoch and trainer.get_num_updates() < max_update and current_bleu < tgt_bleu:
        ...
        prune_print_sparsity(trainer.model, logger=None, show_sparse_only=True, compressed_view=False)

        if not args.quick_save:
...
def save_checkpoint(args, trainer, epoch_itr, val_loss):
    ...
    if args.quick_save:
        ...
        extra_state = {}
        cp = os.path.join(args.save_dir, 'checkpoints','checkpoint{}.pt'.format(epoch))
        trainer.save_checkpoint(cp, extra_state)
        return
    if args.combine_without_finetune:
        extra_state = {}
        save_path = os.path.join(args.save_dir, 'checkpoints','checkpoint_last.pt')
        trainer.save_checkpoint(save_path, extra_state)
        return
...
def load_checkpoint(args, trainer, epoch_itr):
    ...
    if args.resume is not None and args.resume != 'None':
        ...
        if os.path.isfile(checkpoint_path):
            ...
            if extra_state is not None and not args.restart_training:
            ...
            print("Resumed ckpt {} does not exist!".format(checkpoint_path))
            exit()
    else:
        if args.load_parallel_model:
            from torch.serialization import default_restore_location
            print("=> loading multi-partition checkpoint...")
            blk_1_checkpoint = torch.load(args.parallel_1_dir, map_location=lambda s, l: default_restore_location(s, 'cpu'))
            blk_2_checkpoint = torch.load(args.parallel_2_dir, map_location=lambda s, l: default_restore_location(s, 'cpu'))
            blk_3_checkpoint = torch.load(args.parallel_3_dir, map_location=lambda s, l: default_restore_location(s, 'cpu'))
            # blk_4_checkpoint = torch.load(args.parallel_4_dir, map_location=lambda s, l: default_restore_location(s, 'cpu'))
            # blk_5_checkpoint = torch.load(args.parallel_5_dir, map_location=lambda s, l: default_restore_location(s, 'cpu'))
            # blk_6_checkpoint = torch.load(args.parallel_6_dir, map_location=lambda s, l: default_restore_location(s, 'cpu'))

            blk_1_state = blk_1_checkpoint["model"]
            blk_2_state = blk_2_checkpoint["model"]
            blk_3_state = blk_3_checkpoint["model"]
            # blk_4_state = blk_4_checkpoint["model"]
            # blk_5_state = blk_5_checkpoint["model"]
            # blk_6_state = blk_6_checkpoint["model"]
            ...
            for name, W in blk_1_state.items():
                W_cpu = W.cpu().detach().numpy()
                blk_1_detach[name] = torch.from_numpy(W_cpu).float().cuda()
            for name, W in blk_2_state.items():
                W_cpu = W.cpu().detach().numpy()
                blk_2_detach[name] = torch.from_numpy(W_cpu).float().cuda()
            for name, W in blk_3_state.items():
                W_cpu = W.cpu().detach().numpy()
                blk_3_detach[name] = torch.from_numpy(W_cpu).float().cuda()
            # for name, W in blk_4_state.items():
            #     W_cpu = W.cpu().detach().numpy()
            #     blk_4_detach[name] = torch.from_numpy(W_cpu).float().cuda()
            # for name, W in blk_5_state.items():
            #     W_cpu = W.cpu().detach().numpy()
            #     blk_5_detach[name] = torch.from_numpy(W_cpu).float().cuda()
            # for name, W in blk_6_state.items():
            #     W_cpu = W.cpu().detach().numpy()
            #     blk_6_detach[name] = torch.from_numpy(W_cpu).float().cuda()

            partition_set = [blk_1_detach, blk_2_detach, blk_3_detach]

            trainer.combine_sparse_block(partition_set, args)
...
if __name__ == '__main__':
    ...
    '''
    yaml_source_config = "./profiles/6step_0.8.txt"
    yaml_out_folder = "/home/yaml_output_folder/6_step/"
    utils_pr.generate_seq_gap_yaml(yaml_source_config,yaml_out_folder)
    ...
    if ARGS.generate_rand_seq_gap_yaml:
        if ARGS.local_rank == 0:
            utils_pr.generate_rand_seq_gap_yaml(layer_list_file='profiles/transformer_layers.txt', num_part=3, num_yamls=100, output_folder='tmp/')
        exit()
