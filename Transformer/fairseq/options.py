...
def add_distributed_training_args(parser):
    ...
    group.add_argument('--short-train', action='store_true', help='Only train a few minibatches per epoch, for debug purposes')
...
def add_checkpoint_args(parser):
    ...
    group.add_argument('--resume', default=None,
                       help='checkpoint to resume, override auto ckpt filename generation')
    group.add_argument('--restart-training', action='store_true',help='load a ckpt but do not load epoch number and lr')
    group.add_argument('--quick-save', action='store_true',
                       help='do not evaluate model and save best or other info, just save the model')
    return group
...
def add_perf_args(parser):
    ...
    group.add_argument('--apply-att-mask', action='store_true', help='apply masks on attention softmax')
    group.add_argument('--apply-dynamic-att-mask',action='store_true', help='apply dynamic masks on attention after softmax')
    group.add_argument('--mhatt-debug', action='store_true', help='Set breakpoint and debug multiheaded attention module')

    return group

def add_multi_round_args(parser):
    group = parser.add_argument_group('multi_round')
    group.add_argument('--load-parallel-model', action="store_true", dest="load_parallel_model",
                        help="load and combine parallel grown model blocks")
    group.add_argument("--num-gap-partition", default=6, type=int, help="number of partition of gap")
    group.add_argument('--parallel-1-dir', default=None, type=str, help='resnet-50 parallel model 1')
    group.add_argument('--parallel-2-dir', default=None, type=str, help='resnet-50 parallel model 2')
    group.add_argument('--parallel-3-dir', default=None, type=str, help='resnet-50 parallel model 3')
    group.add_argument('--parallel-4-dir', default=None, type=str, help='resnet-50 parallel model 4')
    group.add_argument('--parallel-5-dir', default=None, type=str, help='resnet-50 parallel model 5')
    group.add_argument('--parallel-6-dir', default=None, type=str, help='resnet-50 parallel model 6')
    group.add_argument("--combine-without-finetune", action="store_true",
                        help="hard prune the combine model and save without fine-tune")
