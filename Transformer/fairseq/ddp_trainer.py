...
class DDPTrainer(object):
    ...
    def combine_sparse_block(self, partition_set, args):
        # from prune_utils import utils_pr
        # state_1, state_2 = partition_set[0], partition_set[1]
        # with torch.no_grad():
        #     for name, W in self.model.named_parameters():
        #         name_ = utils_pr.canonical_name(name)
        #         if "encoder." in name_:
        #             W.copy_(state_1[name_])
        #             print("==> migrated layer {}".format(name_))
        #         if "decoder." in name_:
        #             W.copy_(state_2[name_])
        #             print("==> migrated layer {}".format(name_))
        # from prune_utils.admm import ADMM
        # args.sp_admm_update_epoch = 1
        # prune_algo = ADMM(args, self.model, logger=None)
        # prune_algo.prune_harden()
        # return


        # if torch.distributed.get_rank() == 0:
        #     print("weight before combine: {}".format(self.model.state_dict()["module.decoder.layers.0.encoder_attn.in_proj_weight_k"][0][0:5]))
        # print("weight in partition: {}".format(partition_set[3]["decoder.layers.0.encoder_attn.in_proj_weight_k"][0][0:5]))

        # from prune_utils import utils_pr
        # state_1, state_2, state_3, state_4, state_5, state_6 = partition_set[0], partition_set[1], partition_set[2], partition_set[3], partition_set[4], partition_set[5]
        #
        # with torch.no_grad():
        #     for name, W in self.model.named_parameters():
        #         name_ = utils_pr.canonical_name(name)
        #         if "encoder.embed_tokens.weight" in name_ or "encoder.layers.0." in name_ or "encoder.layers.1." in name_:
        #             W.copy_(state_1[name_])
        #             # print("==> migrated layer {}".format(name_))
        #         if "encoder.layers.2." in name_ or "encoder.layers.3." in name_:
        #             W.copy_(state_2[name_])
        #             # print("==> migrated layer {}".format(name_))
        #         if "encoder.layers.4." in name_ or "encoder.layers.5." in name_:
        #             W.copy_(state_3[name_])
        #             # print("==> migrated layer {}".format(name_))
        #         if "decoder.layers.0." in name_ or "decoder.layers.1." in name_:
        #             W.copy_(state_4[name_])
        #             # print("==> migrated layer {}".format(name_))
        #         if "decoder.layers.2." in name_ or "decoder.layers.3." in name_:
        #             W.copy_(state_5[name_])
        #             # print("==> migrated layer {}".format(name_))
        #         if "decoder.layers.4." in name_ or "decoder.layers.5." in name_:
        #             W.copy_(state_6[name_])
        #             # print("==> migrated layer {}".format(name_))

        # if torch.distributed.get_rank() == 0:
        #     print("weight after combine: {}".format(
        #         self.model.state_dict()["module.decoder.layers.0.encoder_attn.in_proj_weight_k"][0][0:5]))

        from prune_utils import utils_pr
        state_1, state_2, state_3 = partition_set[0], partition_set[1], partition_set[2]

        with torch.no_grad():
            for name, W in self.model.named_parameters():
                name_ = utils_pr.canonical_name(name)
                if "encoder.embed_tokens.weight" in name_ or "encoder.layers.0." in name_ or "encoder.layers.1." in name_ or "encoder.layers.2." in name_ or "encoder.layers.3." in name_:
                    W.copy_(state_1[name_])
                    # print("==> migrated layer {}".format(name_))
                if "encoder.layers.4." in name_ or "encoder.layers.5." in name_ or "decoder.layers.0." in name_ or "decoder.layers.1." in name_:
                    W.copy_(state_2[name_])
                    # print("==> migrated layer {}".format(name_))
                if "decoder.layers.2." in name_ or "decoder.layers.3." in name_ or "decoder.layers.4." in name_ or "decoder.layers.5." in name_:
                    W.copy_(state_3[name_])
                    # print("==> migrated layer {}".format(name_))
